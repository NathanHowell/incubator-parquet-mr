...
public class MessageColumnIO extends GroupColumnIO<MessageType> {
  public MessageColumnIO(
  ...
  public <T> RecordReader<T> getRecordReader(PageReadStore columns, RecordMaterializer<T> recordMaterializer) {
    final List<PrimitiveColumnIO> leaves = getLeafColumnIO();
    if (leaves.size() > 0) {
      return new parquet.io.RecordReaderImplementation<T>(
          this,
          recordMaterializer,
          false,
  ...
  public List<ColumnDescriptor> getPhysicalColumnDescriptors() {
    final List<PrimitiveColumnIO> leafs = getLeafColumnIO();
    final ArrayList<ColumnDescriptor> res = new ArrayList<ColumnDescriptor>(leafs.size());
    for (PrimitiveColumnIO leaf : leafs) {
